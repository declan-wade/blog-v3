...
import { Calendar as CalIcon, ChevronLeft, Save, Send, Trash } from 'lucide-react'
    ...
    return (
        <div className="max-w-3xl mx-auto p-6 space-y-6 rounded-lg shadow-md font-[family-name:var(--font-geist-sans)]">
            ...
            <div className="p-8 border rounded-md">
                <Badge className="text-lg font-semibold mb-2 font-[family-name:var(--font-geist-sans)]">Preview</Badge>
            ...
            <footer className="fixed inset-x-0 flex gap-6 p-3 rounded-full place-self-center items-center justify-center outline outline-1 outline-white bottom-10 backdrop-blur-md">
                <Button
                    onClick={() => router.back()}
                    variant="ghost"
                    className="w-20 rounded-md shadow-md transition-all focus:outline-none focus:ring focus:ring-blue-400"
                >
                    <ChevronLeft
                        aria-hidden
                        width={16}
                ...
                <Button
                    onClick={handleSave}
                    variant="ghost"
                    disabled={title.length <= 0 || content.length <= 0 || type.length <= 0 || id != 0}
                    className="w-20 rounded-md shadow-md transition-all focus:outline-none focus:ring focus:ring-blue-400"
                >
                    <Send
                        aria-hidden
